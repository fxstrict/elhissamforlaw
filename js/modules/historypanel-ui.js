...
(function () {
  ...
  function openHistoryPanel() {
    ...
    if (searchInput) {
      // PHASE 30 FIX: this used to be a flat setTimeout(50), which fired
      // while the panel's own slide-in transition (0.25s, css/variables.css
      // --transition) was still less than a quarter finished. Focusing an
      // input opens the on-screen keyboard, which triggers its own
      // viewport-resize animation — running that CONCURRENTLY with the
      // panel's still-in-flight CSS transform/opacity transition produced
      // the torn/partial paint reported (panel visible but its overlay dim
      // never visually completed, both mid-animation at once). Waiting for
      // the panel's actual transitionend means the keyboard only starts
      // animating once the panel's own transition is fully settled — the
      // two animations no longer overlap. Falls back to the old delay if
      // the transition doesn't fire (e.g. prefers-reduced-motion strips it
      // to `transition:none`, which never emits transitionend).
      var focused = false;
      ...
      var panelEl = document.getElementById('historyPanel');
      if (panelEl) {
        panelEl.addEventListener('transitionend', doFocus, { once: true });
      }
      setTimeout(doFocus, 300); // fallback: covers reduced-motion / no-transition environments
    }
  }
